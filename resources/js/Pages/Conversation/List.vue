        ...
        <template #header>
            <div class="flex justify-between">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight my-auto">
                    {{ $t('conversation.conversations') }}</h2>
                <a :href="route('conv.create')"
                   class="bg-red-500 hover:red-700 text-white font-bold py-2 px-4 border border-red-700 rounded">
                    {{ $t('conversation.create_conversation') }}
                </a>
            </div>
        </template>
        <div class="container flex conversation">
            <div class="sidebar bg-gray-200 w-1/3 overflow-hidden overflow-y-auto">
                <div class="chat p-2 border border-gray-300" v-for="conv in conversations" @click="getMessages(conv)">
                    {{ conv.title }}
                </div>
            </div>
            <div class="chat-side flex flex-col w-2/3 border border-gray-300 overflow-hidden overflow-y-auto">
                <div class="messages w-full h-full px-5 justify-between overflow-x-auto">
                    <div class="flex flex-col mt-5" v-for="message in messages">
